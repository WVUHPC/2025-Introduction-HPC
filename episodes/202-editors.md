...
During your interaction with an HPC cluster from the command line, you usually work often with text files.
...
### Exercise: Barnsley_fern

 1. Change your current working directory to your `$SCRATCH` folder

 2. Create a new folder called `FERN`

 3. Open `nano` and enter the following code unde the file `fern.f90`
The code has many opportunities for *copy and paste*, try to use that as much as possible to
familiarize yourself with the editing capabilities of `nano`
...
```
 ...
 4. Load the modules for the GCC compiler. We will learn about modules in the next lecture.
...
```
 ...
 plt.scatter(X, Y, s=0.6, color='g', marker=',');
```
