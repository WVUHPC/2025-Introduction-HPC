...
This is because `~` is a synonimous to the full path to the home directory for the user.*
...
Can I just type `~` instead of `/users/username`?

:::::::::::::::: solution

Yes, it is possible to use `~` as replacement of the full path to your home folder. You can for example execute:
...
But in this case, it is simpler to just execute 

```
~$ cd
```

To go to your home folder. There is also a variable called ``$HOME`` that contains the path to your home folder.
...
### `pwd`, `cd`, and `mkdir`: Folder commands
...
### `cp` and `mv`: Commands for copy and move files and folders
...
### `rm` and `rmdir`: Delete files and Folders
...
|---------|:------------------|:-----------------------------|
| `echo`  | :red_circle: Display a given message on the screen | `$ echo "This is a message"` |
| `cat`   | :red_circle: Display the contents of a file on screen <br> Concatenate files | `$ cat my_file` |
| `date`  | :red_circle: Shows the current date on screen | `$ date` <br> Sun Jul 26 15:41:03 EDT 2020 |
| `pwd`   | :green_circle: Return the path to the current working directory | `$ pwd` <br> /users/username                  |
| `cd`    | :green_circle: Change directory  | `$ cd sub_folder`           |
| `mkdir` | :green_circle: Create directory  | `$ mkdir new_folder`        |
| `touch` | :blue_circle: Change the access and modification time of a file <br> Create empty files     | `$ touch new_file` |
| `cp`    | :purple_circle: Copy a file in another location <br> Copy several files into a destination directory  | `$ cp old_file new_file`  |
| `mv`    | :purple_circle: Move a file in another location <br> Move several files into a destination folder | `$ mv old_name new_name`  |
| `rm`    | :purple_circle: Remove one or more files from the file system tree | `$ rm trash_file` <br> `$ rm -r full_folder`         |
...
Execute the command `cal`, we executed the command before  but this time execute it again like this `cal -y`. What you will get as result?

Another very simple command that is very useful in HPC is `date`.
What is the result of executing this command?
Search for the manual for alternative formats for the date
...
```output
Wed Aug 20 12:50:00 EDT 2025
```

For alternative formats for the date execute:
...
to achieve the same with folders `empty_files` and `dates` with `rm -r`.
